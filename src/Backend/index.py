...
#Route to get all important contact details 
...
#route to fetch suspect summary data
...
def index3():
    if request.method == "GET":
        ...
        query = "select COUNT(p.Suspect_ID),l.AppDisplayName, l.location, l.GLocation,l.location_id from suspect p, location l where p.location_id=l.location_id group by l.location_id"
        cur.execute(query )
        data = cur.fetchall()
        cur.close()
        return jsonify(data)
    
#route to report suspect    
...
#route to get suspect data of given locaion    
